...
function Show-Menu {
    ...
    foreach ($sec in $sections) {
        ...
        for ($i = 0; $i -lt $sec.Keys.Count; $i += $chunkSize) {
            $group = $sec.Keys[$i..([Math]::Min($i + $chunkSize - 1, $sec.Keys.Count - 1))]
            $line  = ''
            foreach ($k in $group) {
                $line += "[{0}] {1,-18}" -f $k, $menuMap[$k]
            }
...
function Get-EsptoolPath {
    ...
    if (Get-Command esptool.exe -ErrorAction SilentlyContinue) {
...
function Get-EspefusePath {
    ...
    if (Get-Command espefuse.exe -ErrorAction SilentlyContinue) {
...
function Get-SerialPort {
    while ($true) {
        ...
        if ($ports.Count -eq 0) {
            Write-Host "No COM ports found. Connect your ESP device and press any key to refresh..." -ForegroundColor Yellow
...
function Invoke-Esptool {
    ...
    $cmd = "$script:esptoolPath {0}" -f ($EsptoolArgs -join ' ')
...
function Invoke-Espefuse {
    [CmdletBinding()]
    param([string[]]$EspefuseArgs)
    $cmd = "$script:espefusePath {0}" -f ($EspefuseArgs -join ' ')
    Write-Host "Running: $cmd" -ForegroundColor Cyan
    ...
    switch ($selInt) {
        ...
        3  {
            if (Confirm-Action -Message 'Are you sure you want to Erase Flash?') {
                ...
                Write-Host 'Erase Flash canceled.' -ForegroundColor $script:UIColors.Warning
            }
        }
        4  {
            ...
            if (Test-Path $fwFolder) {
                ...
                $app  = TryGetBinPath -Bins $bins -Pattern '^(firm|app)'-Label 'firmware'
        ...
        5  {
            $start = Read-Host "Start offset (hex, e.g. 0x00000)"
            $len   = Read-Host "Length (decimal)"
            $out   = Read-Host "Output filename"
            Invoke-Esptool -EsptoolArgs @(
                '--chip',$chip,'--port',$port,
                'dump-flash','--start',$start,'--length',$len,
                $out
            )
        }
        6  { Invoke-Esptool -EsptoolArgs @('--chip',$chip,'--port',$port,'read-mac') }
        7  {
            $mac = Read-Host "Enter new MAC (xx:xx:xx:xx:xx:xx)"
            Invoke-Esptool -EsptoolArgs @('--chip',$chip,'--port',$port,'write-mac',$mac)
        }
        8  { Invoke-Espefuse -EspefuseArgs @('--chip',$chip,'--port',$port,'summary') }
        9  {
            if (Confirm-Action -Message 'Are you sure you want to Burn eFuse?') {
                $efuse = Read-Host "Enter eFuse to burn (e.g. DIS_USB_JTAG)"
                Invoke-Espefuse -EspefuseArgs @('--chip',$chip,'--port',$port,'burn-efuse',$efuse)
            } else {
                Write-Host 'Burn eFuse canceled.' -ForegroundColor $script:UIColors.Warning
            }
        }
       10 {
            $addr = Read-Host "Memory address (hex)"
            $cnt  = Read-Host "Byte count (decimal)"
            $file = Read-Host "Output filename"
            Invoke-Esptool -EsptoolArgs @('--chip',$chip,'--port',$port,'read-memory',$addr,$cnt,$file)
        }
       11 {
            $addr = Read-Host "Memory address (hex)"
            $file = Read-Host "Input filename"
            Invoke-Esptool -EsptoolArgs @('--chip',$chip,'--port',$port,'write-memory',$addr,$file)
        }
       12 {
            $addr = Read-Host "RAM address to load stub (hex)"
            $stub = Read-Host "Stub filename to load"
            Invoke-Esptool -EsptoolArgs @('--chip',$chip,'--port',$port,'load-ram',$addr,$stub)
        }
